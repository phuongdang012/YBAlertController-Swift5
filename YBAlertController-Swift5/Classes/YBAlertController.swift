...
public class YBAlertController: UIViewController, UIGestureRecognizerDelegate {
    ...
    public init() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(self.dismiss(sender:)))
        tapGesture.numberOfTapsRequired = 1
        tapGesture.delegate = self
        self.view.addGestureRecognizer(tapGesture)
        
        NotificationCenter.default.addObserver(self, selector: #selector(changedOrientation(notification:)), name: UIDevice.orientationDidChangeNotification, object: nil)
    }
    ...
    @objc func dismiss(sender: Any? = nil) {
        showing = false
...
//            })
            let time = DispatchTime.now() + DispatchTimeInterval.nanoseconds(Int(delay * Double(i)))
